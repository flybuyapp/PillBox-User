...
import '../../../widgets/sleep_widgets/uniform_box_decoration.dart';
import '../../../widgets/sleep_widgets/wakeup_list.dart';
...
class _WakeUpScreen extends State<WakeUpScreen> {
  ...
  void getDataFromPreferenceAndDatabase() async {
    ...
    if (allSleeps.length > 0) {
      for (var sleep in allSleeps) {
        totalDuration += int.parse((sleep.duration / 1000).toString());
  ...
  /*Future<void> fetchWeather(double lat, double lon) async {
    final response = await http.get(Uri.parse("https://api.openweathermap.org/data/2.5/weather?lat=${lat.round()}&lon=${lon.round()}&appid=$apiKey&units=$_units"));
